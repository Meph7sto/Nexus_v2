...
type UserQuery struct {
	...
	withAdminPermissions      *AdminPermissionQuery
...
// QueryAdminPermissions chains the current query on the "admin_permissions" edge.
func (_q *UserQuery) QueryAdminPermissions() *AdminPermissionQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAdminPermissions:      _q.withAdminPermissions.Clone(),
...
// WithAdminPermissions tells the query-builder to eager-load the nodes that are connected to
// the "admin_permissions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAdminPermissions(opts ...func(*AdminPermissionQuery)) *UserQuery {
	query := (&AdminPermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAdminPermissions = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAdminPermissions; query != nil {
		if err := _q.loadAdminPermissions(ctx, query, nodes,
			func(n *User) { n.Edges.AdminPermissions = []*AdminPermission{} },
			func(n *User, e *AdminPermission) { n.Edges.AdminPermissions = append(n.Edges.AdminPermissions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadAdminPermissions(ctx context.Context, query *AdminPermissionQuery, nodes []*User, init func(*User), assign func(*User, *AdminPermission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(adminpermission.FieldUserID)
	}
	query.Where(predicate.AdminPermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AdminPermissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
