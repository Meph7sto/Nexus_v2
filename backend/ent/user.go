...
type UserEdges struct {
	...
	// AdminPermissions holds the value of the admin_permissions edge.
...
// AdminPermissionsOrErr returns the AdminPermissions value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) AdminPermissionsOrErr() ([]*AdminPermission, error) {
	if e.loadedTypes[13] {
		return e.AdminPermissions, nil
	}
	return nil, &NotLoadedError{edge: "admin_permissions"}
}
...
// QueryAdminPermissions queries the "admin_permissions" edge of the User entity.
func (_m *User) QueryAdminPermissions() *AdminPermissionQuery {
	return NewUserClient(_m.config).QueryAdminPermissions(_m)
}
