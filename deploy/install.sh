...
# Nexus 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Wei-Shaw/nexus/main/deploy/install.sh | bash
#
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="Nexus 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Nexus 服务："
    ...
    ["not_installed"]="Nexus 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Nexus。"
    ...
    ["uninstall_complete"]="Nexus 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Nexus"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Nexus"
    ...
    ["server_config_desc"]="配置 Nexus 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Nexus Installation Script"
    ...
    # Completion
    ["install_complete"]="Nexus installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Nexus service:"
    ...
    ["not_installed"]="Nexus is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Nexus from your system."
    ...
    ["uninstall_complete"]="Nexus has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Nexus"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Nexus"
    ...
    ["server_config_desc"]="Configure Nexus server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/nexus" ]; then
        ...
        "$INSTALL_DIR/nexus" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="nexus_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/nexus" "$INSTALL_DIR/nexus"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/nexus"
}

# Create system user
create_user() {
    if ! getent group "$SERVICE_USER" >/dev/null 2>&1; then
        if getent group sub2api >/dev/null 2>&1; then
            local legacy_gid
            legacy_gid=$(getent group sub2api | cut -d: -f3)
            groupadd -r -o -g "$legacy_gid" "$SERVICE_USER"
        else
    ...
    if id "$SERVICE_USER" &>/dev/null; then
        ...
        if id sub2api >/dev/null 2>&1; then
            local legacy_uid
            legacy_uid=$(id -u sub2api)
            useradd -r -o -u "$legacy_uid" -g "$SERVICE_USER" -s /bin/sh -d "$INSTALL_DIR" "$SERVICE_USER"
        else
            useradd -r -g "$SERVICE_USER" -s /bin/sh -d "$INSTALL_DIR" "$SERVICE_USER"
        fi
        print_success "$(msg 'user_created')"
    fi
}

# Adopt an existing native installation without moving or renaming its data.
# Nexus and the legacy service share the existing UID/GID, and the new data
# path is a symlink so rollback can still use the original layout.
adopt_legacy_install() {
    if [ -f "$INSTALL_DIR/nexus" ] || [ ! -f "$LEGACY_INSTALL_DIR/sub2api" ]; then
        return 0
    fi

    print_info "Adopting existing installation from $LEGACY_INSTALL_DIR"
    mkdir -p "$INSTALL_DIR"
    cp "$LEGACY_INSTALL_DIR/sub2api" "$INSTALL_DIR/nexus"
    chmod +x "$INSTALL_DIR/nexus"
    if [ -d "$LEGACY_INSTALL_DIR/data" ] && [ ! -e "$INSTALL_DIR/data" ]; then
        ln -s "$LEGACY_INSTALL_DIR/data" "$INSTALL_DIR/data"
    fi
...
[Unit]
Description=Nexus - AI API Gateway Platform
...
start_service() {
    ...
    if systemctl start nexus; then
        ...
        print_info "sudo journalctl -u nexus -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable nexus 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status nexus"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u nexus -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart nexus"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop nexus"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/nexus" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    ...
    cp "$INSTALL_DIR/nexus" "$INSTALL_DIR/nexus.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/nexus.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/nexus"
...
# Requires: Nexus must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/nexus" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="nexus.backup.${current_version}"
        else
            backup_name="nexus.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/nexus" "$INSTALL_DIR/$backup_name"
