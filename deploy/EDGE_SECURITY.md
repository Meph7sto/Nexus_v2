...
`security.trust_forwarded_ip_for_api_key_acl` is disabled by default for new
Nexus installations. It may be enabled during an upgrade for compatibility
when no explicit trusted-proxy policy exists. While enabled, raw forwarding
headers take over client-IP
...
CIDR/IP addresses that connect directly to Nexus. An explicit empty list
