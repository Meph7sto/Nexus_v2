...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/nexus/repo
...
参数:
  ...
  --source  指定 nexus 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助
...
if ! getent group nexus >/dev/null 2>&1; then
  if getent group sub2api >/dev/null 2>&1; then
    groupadd --system --non-unique --gid "$(getent group sub2api | cut -d: -f3)" nexus
  else
...
if ! id nexus >/dev/null 2>&1; then
  ...
  if id sub2api >/dev/null 2>&1; then
    useradd --system --non-unique --uid "$(id -u sub2api)" --gid nexus --no-create-home --shell /usr/sbin/nologin nexus
  else
    useradd --system --gid nexus --no-create-home --shell /usr/sbin/nologin nexus
  fi
else
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R nexus:nexus /var/lib/sub2api
chmod 0750 "$DATA_DIR"
