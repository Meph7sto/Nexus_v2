<template>
  <section aria-labelledby="prompt-events-title" class="py-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 id="prompt-events-title" class="text-base font-semibold text-gray-950 ">{{ t('admin.promptAudit.events.title') }}</h2>
        <p class="mt-1 text-sm text-gray-500 ">{{ t('admin.promptAudit.events.description') }}</p>
      </div>
    ...
    <div class="mt-5 overflow-x-auto rounded-lg border border-gray-200 ">
      <table class="min-w-[1120px] w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-gray-100 bg-white  ">
          ...
          <tr v-for="event in events" v-else :key="event.id" :data-test="`event-${event.id}`" class="align-top hover:bg-gray-50">
            ...
            <td class="whitespace-nowrap px-3 py-3 text-xs text-gray-600 ">{{ formatDate(event.created_at) }}</td>
            <td class="px-3 py-3">
            ...
            <td class="px-3 py-3 text-gray-700 ">{{ event.snapshot.group_name || '—' }}</td>
            <td class="px-3 py-3">
              <p class="font-medium text-gray-900 ">{{ event.snapshot.endpoint }}</p>
            ...
            <td class="max-w-xs px-3 py-3"><p class="line-clamp-2 break-words text-gray-600 ">{{ event.snapshot.redacted_preview || '—' }}</p></td>
...
const FilterInput = defineComponent({
  ...
  setup(componentProps, { emit: componentEmit }) {
    return () => h('label', { class: 'text-xs text-gray-600 ' }, [
...
const CopyLine = defineComponent({
  ...
  setup(componentProps) {
    return () => h('div', { class: 'flex max-w-56 items-center gap-1 text-xs' }, [
      ...
      h('span', { class: 'min-w-0 flex-1 truncate text-gray-800 ' }, componentProps.value || '—'),
