<template>
  <section aria-labelledby="prompt-policy-title" class="py-6">
    <div>
      <h2 id="prompt-policy-title" class="text-base font-semibold text-gray-950 ">{{ t('admin.promptAudit.policy.title') }}</h2>
      <p class="mt-1 text-sm text-gray-500 ">{{ t('admin.promptAudit.policy.description') }}</p>
    </div>
    ...
    <div class="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.45fr)]">
      <div class="rounded-lg border border-gray-200 p-4   sm:p-5">
        <fieldset>
          <legend class="text-sm font-medium text-gray-900 ">{{ t('admin.promptAudit.policy.scope') }}</legend>
        ...
        <div v-if="!draft.all_groups" class="mt-4">
          ...
          <div class="mt-3 max-h-52 overflow-y-auto rounded-lg border border-gray-200 p-2 ">
            <label v-for="group in filteredGroups" :key="group.id" class="flex cursor-pointer items-center justify-between gap-3 rounded-md px-2 py-2 text-sm hover:bg-gray-50 ">
              ...
              <span class="text-xs text-gray-500 ">{{ group.platform }} · {{ group.status }}</span>
            </label>
          ...
          <p class="mt-2 text-xs text-gray-500 ">{{ t('admin.promptAudit.policy.selectedCount', { count: draft.group_ids.length }) }}</p>
        </div>

        <fieldset class="mt-5 border-t border-gray-100 pt-5 ">
          <legend class="text-sm font-medium text-gray-900 ">{{ t('admin.promptAudit.policy.scanners') }}</legend>
          <div class="mt-3 grid gap-2 sm:grid-cols-2">
            <label v-for="scanner in SCANNER_CATALOG" :key="scanner.id" class="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-gray-700 hover:bg-gray-50  ">
      ...
      <div class="space-y-4 rounded-lg border border-gray-200 p-4   sm:p-5">
        <label class="block text-sm text-gray-700 ">
        ...
        <div class="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-600  ">
          <p class="font-medium text-gray-800 ">{{ t('admin.promptAudit.policy.strategy') }}</p>
