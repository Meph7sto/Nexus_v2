<template>
  <section aria-labelledby="prompt-pool-title" class="border-b border-gray-200 py-6 ">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 id="prompt-pool-title" class="text-base font-semibold text-gray-950 ">{{ t('admin.promptAudit.pool.title') }}</h2>
        <p class="mt-1 text-sm text-gray-500 ">{{ t('admin.promptAudit.pool.description') }}</p>
      </div>
    ...
    <div v-if="endpoints.length === 0" class="mt-5 rounded-lg border border-dashed border-gray-300 px-5 py-10 text-center text-sm text-gray-500   ">
      {{ t('admin.promptAudit.pool.empty') }}
    </div>
    <div v-else class="mt-5 overflow-hidden rounded-lg border border-gray-200 bg-white  ">
      <div class="hidden grid-cols-[minmax(260px,1.45fr)_minmax(210px,1fr)_minmax(190px,.8fr)_minmax(230px,1.15fr)_auto] gap-5 border-b border-l-[3px] border-b-gray-200 border-l-transparent bg-gray-50 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-gray-500 xl:grid">
      ...
      <div class="divide-y divide-gray-100 ">
        <article
          ...
          class="group grid gap-4 border-l-[3px] border-l-transparent px-4 py-4 transition-[background-color,border-color] duration-200 hover:border-l-primary-500 hover:bg-gray-50 sm:px-5 xl:grid-cols-[minmax(260px,1.45fr)_minmax(210px,1fr)_minmax(190px,.8fr)_minmax(230px,1.15fr)_auto] xl:items-center xl:gap-5"
        >
          <div class="flex min-w-0 items-center gap-3">
            <AdminPermissionGate resource="prompt_audit" action="update">
              <button
                ...
                :class="endpoint.enabled ? 'bg-primary-600' : 'bg-gray-200 '"
            ...
            <div class="min-w-0">
              <div class="flex min-w-0 items-center gap-2">
                <p class="truncate font-semibold text-gray-950 ">{{ endpoint.name }}</p>
                <span class="h-1.5 w-1.5 shrink-0 rounded-full" :class="endpoint.enabled ? 'bg-emerald-500' : 'bg-gray-300 '" aria-hidden="true" />
              </div>
              <p class="mt-0.5 truncate font-mono text-[11px] text-gray-500 " :title="endpoint.base_url">{{ endpoint.base_url }}</p>
            </div>
          </div>
          ...
          <div class="min-w-0 xl:block">
            ...
            <p class="truncate text-sm font-medium text-gray-700 " :title="endpoint.model">{{ endpoint.model }}</p>
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1.5 text-xs text-gray-600 ">
              <span class="rounded-md bg-gray-100 px-2 py-1 tabular-nums ">{{ endpoint.timeout_ms }} ms</span>
              <span class="rounded-md bg-gray-100 px-2 py-1 tabular-nums ">{{ endpoint.input_limit }} chars</span>
            </div>
          </div>
          ...
          <div class="min-w-0">
            ...
            <div class="flex items-center gap-1.5 text-xs font-medium" :class="hasCredential(endpoint) ? 'text-emerald-700 ' : 'text-gray-500 '">
              <span class="h-1.5 w-1.5 rounded-full" :class="hasCredential(endpoint) ? 'bg-emerald-500' : 'bg-gray-300 '" aria-hidden="true" />
              {{ hasCredential(endpoint) ? t('admin.promptAudit.pool.configured') : t('admin.promptAudit.pool.missing') }}
            </div>
            <p v-if="probingIds.includes(endpoint.id)" class="mt-1.5 text-xs text-primary-600 ">
          ...
          <div class="flex flex-wrap items-center justify-end gap-1 border-t border-gray-100 pt-3  xl:flex-nowrap xl:border-0 xl:pt-0">
    ...
    <BaseDialog :show="Boolean(editing)" :title="editingIndex < 0 ? t('admin.promptAudit.pool.add') : t('admin.promptAudit.pool.edit')" width="wide" @close="closeEditor">
      <form v-if="editing" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveEditor">
        ...
        <label class="space-y-1 text-sm text-gray-700  sm:col-span-2">
        ...
        <label class="space-y-1 text-sm text-gray-700  sm:col-span-2">
          ...
          <span class="block text-xs text-gray-500 ">{{ t('admin.promptAudit.pool.secretHint') }}</span>
        </label>
