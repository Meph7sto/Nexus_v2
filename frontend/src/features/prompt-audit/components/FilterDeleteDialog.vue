<template>
  <BaseDialog :show="show" :title="t('admin.promptAudit.events.filterDeleteDialogTitle')" width="wide" @close="$emit('close')">
    <div class="space-y-5 text-sm">
      <p class="text-gray-500 ">{{ t('admin.promptAudit.events.filterDeleteDialogDesc') }}</p>

      <fieldset>
        <legend class="text-xs font-medium text-gray-600 ">{{ t('admin.promptAudit.events.filterTimeRange') }}</legend>
        <div class="mt-2 flex flex-wrap gap-2" role="radiogroup" :aria-label="t('admin.promptAudit.events.filterTimeRange')">
          ...
          >
            ...
            <span class="inline-flex items-center rounded-full border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-colors peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700 peer-focus-visible:ring-2 peer-focus-visible:ring-red-500/30     ">
              {{ t(`admin.promptAudit.events.timePresets.${option.id}`) }}
            </span>
          </label>
        </div>
        <p class="mt-2 text-xs text-gray-500 ">{{ t('admin.promptAudit.events.filterTimeRangeHint') }}</p>
        <div v-if="preset === 'custom'" class="mt-3 grid gap-3 sm:grid-cols-2" data-test="custom-range">
          ...
          <p v-if="!canPreview" class="text-xs text-red-600  sm:col-span-2">{{ t('admin.promptAudit.events.customRangeInvalid') }}</p>
        </div>
      </fieldset>
      ...
      <details class="rounded-lg border border-gray-200 px-4 py-3 " data-test="more-conditions">
        <summary class="cursor-pointer select-none text-xs font-medium text-gray-600 ">{{ t('admin.promptAudit.events.moreConditions') }}</summary>
      ...
      <div v-if="preview" class="rounded-lg border border-red-200 bg-red-50/60 px-4 py-3  " data-test="delete-preview-result">
        <p class="text-sm font-semibold text-red-700 ">{{ t('admin.promptAudit.events.filterDeleteCount', { count: preview.matched_count }) }}</p>
        <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs text-gray-600 ">
        ...
        <p class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800  ">{{ t('admin.promptAudit.events.filterDeleteWarning') }}</p>
      </div>
      <p v-else class="rounded-lg border border-dashed border-gray-300 px-4 py-3 text-xs text-gray-500  " data-test="delete-preview-empty">
    ...
    <template #footer>
      <div class="flex flex-wrap items-center justify-end gap-3">
        <p v-if="confirmDisabledReason" class="mr-auto text-xs text-gray-500 " data-test="confirm-disabled-reason">
