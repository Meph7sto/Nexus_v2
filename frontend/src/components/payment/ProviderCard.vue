<template>
  <div
    :class="[
      ...
      enabled ? 'border-gray-200 ' : 'border-gray-200 bg-gray-50 opacity-50  ',
    ]"
  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-3">
        <div :class="[
          'rounded-md p-1.5',
          provider.enabled && enabled ? 'bg-green-100 ' : 'bg-gray-100 ',
        ]">
          <Icon
            name="server"
            size="sm"
        ...
        <div v-if="enabled" class="flex items-center gap-1">
          <AdminPermissionGate v-for="pt in availableTypes" :key="pt.value" resource="payment_providers" action="update">
            <button
              type="button"
      ...
      <div class="flex items-center gap-4">
        <AdminPermissionGate resource="payment_providers" action="update">
        ...
        <AdminPermissionGate v-if="provider.refund_enabled" resource="payment_providers" action="update">
          <ToggleSwitch :label="t('admin.settings.payment.allowUserRefund')" :checked="provider.allow_user_refund" @toggle="emit('toggleField', 'allow_user_refund')" />
        </AdminPermissionGate>
        <div class="flex items-center gap-2 border-l border-gray-200 pl-3 ">
          <AdminPermissionGate resource="payment_providers" action="update">
            <button type="button" @click="emit('edit')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-blue-50 hover:text-blue-600  ">
