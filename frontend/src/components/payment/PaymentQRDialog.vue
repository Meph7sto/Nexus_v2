  ...
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      ...
      <template v-if="qrUrl">
        ...
        <p v-if="scanHint" class="text-center text-sm text-gray-500 ">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-gray-500 ">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-gray-500 ">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
    ...
    <div v-else class="flex flex-col items-center space-y-4 py-4">
      ...
      <div v-if="paidOrder" class="w-full rounded-lg bg-gray-50 p-4 ">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-gray-900 ">{{ creditedAmountSymbol }}{{ paidOrder.amount.toFixed(2) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500 ">{{ t('payment.orders.payAmount') }}</span>
            <span class="font-medium text-gray-900 ">{{ paymentAmountSymbol(paidOrder) }}{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
