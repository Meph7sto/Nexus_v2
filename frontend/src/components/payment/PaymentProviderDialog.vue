    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-gray-500 ">{{ t('admin.settings.payment.paymentMode') }}</span>
        ...
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-gray-500 ">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div v-if="form.provider_key === 'easypay'" class="space-y-3 rounded-lg border border-gray-100 p-3 ">
        ...
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            <div>
              <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.customMethodType') }}</label>
              <input v-model="method.type" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.customMethodUpstreamType') }}</label>
              <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.customMethodDisplayName') }}</label>
      ...
      <div class="border-t border-gray-200 pt-4 ">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-gray-300 text-[11px] font-semibold text-gray-400 transition-colors hover:border-primary-500 hover:text-primary-600    "
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-gray-500 ">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-gray-500 ">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-gray-500   ">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-xs text-gray-500   ">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-if="limitableTypes.length" class="border-t border-gray-200 pt-4 ">
        ...
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          ...
          >
            <p class="mb-2 text-xs font-medium text-gray-700 ">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-gray-500 ">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-gray-400 ">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
