<template>
  <button
    type="button"
    class="group text-left p-5 rounded-lg min-h-[280px] w-full bg-white/70  border border-gray-200/80 shadow-card   hover:-translate-y-1 hover:shadow-card-hover  hover:border-gray-300 transition-all duration-300 ease-out flex flex-col"
    @click="emit('click')"
  >
    <!-- Header: icon + name/model + status chip -->
    <div class="flex items-start gap-3">
      <span
        class="w-9 h-9 rounded-lg ring-1 ring-black/5  grid place-items-center flex-shrink-0"
        :class="[providerTileClass(item.provider), providerTintClass]"
      >
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          <span
            ...
            class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-600   flex-shrink-0"
          >
            {{ item.group_name }}
          </span>
        </div>
