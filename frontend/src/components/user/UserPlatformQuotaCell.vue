<template>
  <span v-if="props.quotas === undefined" class="text-xs text-gray-400 ">…</span>
  <span v-else-if="configured.length === 0" class="text-xs text-gray-400 ">
  ...
  <div v-else class="space-y-0.5 text-xs">
    ...
    >
      <span class="w-20 shrink-0 font-mono text-gray-700 ">{{ row.platform }}</span>
      <span class="text-gray-500 ">
        {{ t('admin.users.platformQuota.windowDaily') }}
        <span class="text-gray-900 ">{{ fmtUsd(row.daily_usage_usd) }}/{{ fmtLimit(row.daily_limit_usd) }}</span>
      </span>
      <span class="text-gray-500 ">
        {{ t('admin.users.platformQuota.windowWeekly') }}
        <span class="text-gray-900 ">{{ fmtUsd(row.weekly_usage_usd) }}/{{ fmtLimit(row.weekly_limit_usd) }}</span>
      </span>
      <span class="text-gray-500 ">
        {{ t('admin.users.platformQuota.windowMonthly') }}
        <span class="text-gray-900 ">{{ fmtUsd(row.monthly_usage_usd) }}/{{ fmtLimit(row.monthly_limit_usd) }}</span>
      </span>
    </div>
  </div>
</template>
