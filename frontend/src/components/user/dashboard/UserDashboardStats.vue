<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.apiKeys') }}</p>
          <p class="text-xl font-bold text-gray-900 ">{{ stats?.total_api_keys || 0 }}</p>
          <p class="text-xs text-green-600 ">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.todayRequests') }}</p>
          <p class="text-xl font-bold text-gray-900 ">{{ stats?.today_requests || 0 }}</p>
          <p class="text-xs text-gray-500 ">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-purple-100 p-2 ">
          <Icon name="dollar" size="md" class="text-purple-600 " :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.todayCost') }}</p>
          <p class="text-xl font-bold text-gray-900 ">
            <span class="text-purple-600 " :title="t('dashboard.actual')">${{ formatCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-sm font-normal text-gray-400 " :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 ">{{ t('common.total') }}: </span>
            <span class="text-purple-600 " :title="t('dashboard.actual')">${{ formatCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-gray-400 " :title="t('dashboard.standard')"> / ${{ formatCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.todayTokens') }}</p>
          <p class="text-xl font-bold text-gray-900 ">{{ formatTokens(stats?.today_tokens || 0) }}</p>
          <p class="text-xs text-gray-500 ">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.totalTokens') }}</p>
          <p class="text-xl font-bold text-gray-900 ">{{ formatTokens(stats?.total_tokens || 0) }}</p>
          <p class="text-xs text-gray-500 ">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.performance') }}</p>
          <div class="flex items-baseline gap-2">
            <p class="text-xl font-bold text-gray-900 ">{{ formatTokens(stats?.rpm || 0) }}</p>
            <span class="text-xs text-gray-500 ">RPM</span>
          </div>
          <div class="flex items-baseline gap-2">
            <p class="text-sm font-semibold text-violet-600 ">{{ formatTokens(stats?.tpm || 0) }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-medium text-gray-500 ">{{ t('dashboard.avgResponse') }}</p>
          <p class="text-xl font-bold text-gray-900 ">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
          <p class="text-xs text-gray-500 ">{{ t('dashboard.averageTime') }}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-if="!isSimple && platformCards.length > 0" class="card p-4">
    <div class="mb-3 flex items-center justify-between">
      <h3 class="text-sm font-semibold text-gray-900 ">{{ t('dashboard.platformBreakdown') }}</h3>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-purple-600 " :title="t('dashboard.actual')">
            ${{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 ">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 ">${{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500 ">{{ t('dashboard.requests') }}</span>
            <span class="font-mono text-gray-700 ">
              {{ item.total_requests > 0 ? formatNumber(item.total_requests) : '-' }}
            </span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500 ">{{ t('dashboard.tokens') }}</span>
            <span class="font-mono text-gray-700 ">
        ...
        <div v-if="hasAnyLimit(item.quota) && !item.isOther" class="mt-3 space-y-1.5 border-t border-gray-200 pt-2 ">
            ...
            <div v-if="quotaVal(item.quota, `${w}_limit_usd`) != null" class="space-y-0.5">
              ...
              <template v-if="(quotaVal(item.quota, `${w}_limit_usd`) as number) === 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-gray-600 ">{{ t(`dashboard.platformQuota.${w}`) }}</span>
