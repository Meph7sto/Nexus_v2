    ...
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="rounded-lg bg-gray-50 p-3 ">
        ...
        <div class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-gray-900 ">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 ">{{ t('payment.orders.payAmount') }}</span>
          <span class="font-medium text-gray-900 ">{{ paymentAmountSymbol }}{{ order?.pay_amount?.toFixed(2) }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 ">{{ t('payment.admin.alreadyRefunded') }}</span>
          <span class="font-medium text-red-600 ">{{ creditedAmountSymbol }}{{ actuallyRefunded.toFixed(2) }}</span>
        </div>
      </div>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          />
          <label for="deduct-balance" class="text-sm text-gray-700 ">
            {{ t('payment.admin.deductBalance') }}
          </label>
          <span class="text-xs text-gray-500 ">{{ t('payment.admin.deductBalanceHint') }}</span>
        </div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          <div class="rounded-lg bg-gray-50 p-3 text-sm ">
            <div class="text-gray-500 ">{{ t('payment.admin.userBalance') }}</div>
            <div class="mt-1 font-semibold text-gray-900 ">{{ creditedAmountSymbol }}{{ userBalance.toFixed(2) }}</div>
          </div>
          <div class="rounded-lg bg-gray-50 p-3 text-sm ">
            <div class="text-gray-500 ">{{ t('payment.admin.orderAmount') }}</div>
            <div class="mt-1 font-semibold text-gray-900 ">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</div>
          </div>
        </div>
        ...
        <div
          ...
          class="mt-2 rounded-lg bg-amber-50 p-3 text-sm text-amber-700  "
        >
        ...
        <div
          ...
          class="mt-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-700  "
        >
      ...
      <div v-if="requireForce" class="flex items-center gap-2">
        ...
        />
        <label for="force-refund" class="text-sm font-medium text-red-600 ">
