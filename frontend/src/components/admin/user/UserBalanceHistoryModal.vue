    ...
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-lg bg-gray-50 p-4 ">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-gray-500 ">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 ">
          <p class="min-w-0 flex-1 truncate text-xs text-gray-500 " :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 ">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-emerald-600 ">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-50    "
        >
