<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-lg bg-primary-50 p-5  ">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm ">
          <span class="text-2xl font-semibold text-primary-600 ">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-gray-900 ">{{ user.email }}</p>
          <p class="mt-1 text-sm text-gray-600 ">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-gray-700 ">{{ t('admin.users.exclusiveGroups') }}</h4>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-gray-300 transition-all peer-checked:border-primary-500 peer-checked:bg-primary-500  peer-checked:">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-gray-900 ">{{ config.groupName }}</span>
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    ...
                    <span class="text-gray-500 ">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-gray-700 ">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-gray-600 ">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20   "
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-gray-700 ">{{ t('admin.users.publicGroups') }}</h4>
          ...
          <div class="grid gap-3">
            <div
              ...
              :key="config.groupId"
              class="relative overflow-hidden rounded-lg border-2 border-green-200 bg-green-50/50 p-4  "
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-green-400 bg-green-500  ">
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p class="text-gray-500 ">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
