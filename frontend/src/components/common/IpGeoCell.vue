<template>
  <div v-if="entry.status === 'idle'" class="mt-0.5 text-xs">
    <button
      type="button"
      class="text-primary-600 underline decoration-dashed underline-offset-2 hover:text-primary-700  "
      @click="handleFetch"
    >
      {{ t('usage.ipGeo.fetch') }}
    </button>
  ...
  <div v-else-if="entry.status === 'success'" class="mt-0.5 flex items-center gap-1 text-xs">
    <button
      type="button"
      class="truncate text-gray-500 underline decoration-dotted underline-offset-2 hover:text-primary-600  "
      :title="tooltipText"
