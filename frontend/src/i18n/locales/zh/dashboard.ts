export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description: '配置 Grok Build、Claude Code、Codex 或 OpenCode，让请求通过当前 Nexus Grok 分组发送。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 Nexus Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 Nexus Grok 分组发送。',
        ...
        codexNote: '将 config.toml 保存到 ~/.codex，并在启动 Codex 前设置 NEXUS_API_KEY。',
        codexNoteWindows: '将 config.toml 保存到 %USERPROFILE%\\.codex，并在 PowerShell 中设置 NEXUS_API_KEY 后启动 Codex。'
      },
      opencode: {
        title: 'OpenCode 配置示例',
