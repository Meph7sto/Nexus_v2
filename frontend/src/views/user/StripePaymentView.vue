<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 ' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 ">{{ t('payment.stripeLoadFailed') }}</h3>
      ...
      <template v-else>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-gray-900 ">{{ t('payment.qr.scanWxpay') }}</p>
              <div class="relative rounded-lg border-2 border-[#2BB741] bg-green-50 p-4  ">
              ...
              <p class="text-center text-sm text-gray-500 ">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-gray-500 ">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-lg font-bold text-gray-900 ">{{ t('payment.result.success') }}</p>
              <p class="text-sm text-gray-500 ">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-red-600 ">{{ stripeError }}</p>
...
function mountPaymentElement(stripe: Stripe, clientSecret: string) {
  const elements = stripe.elements({
    clientSecret,
    appearance: { theme: 'stripe', variables: { borderRadius: '4px' } },
  })
