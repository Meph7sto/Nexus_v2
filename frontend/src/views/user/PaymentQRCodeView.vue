<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <div v-if="qrUrl" class="rounded-lg bg-white p-6 shadow-card ">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-gray-500 ">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-gray-500 ">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-gray-900 ">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-gray-400 ">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
