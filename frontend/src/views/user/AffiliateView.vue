    ...
    <div class="space-y-6">
      ...
      <template v-else-if="detail">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div class="card p-5">
            <p class="text-sm text-gray-500 ">{{ t('affiliate.stats.invitedUsers') }}</p>
            <p class="mt-2 text-2xl font-semibold text-gray-900 ">
              {{ formatCount(detail.aff_count) }}
            </p>
          </div>
          <div class="card p-5">
            <p class="text-sm text-gray-500 ">{{ t('affiliate.stats.availableQuota') }}</p>
            <p class="mt-2 text-2xl font-semibold text-emerald-600 ">
              {{ formatCurrency(detail.aff_quota) }}
            </p>
          </div>
          <div class="card p-5">
            <p class="text-sm text-gray-500 ">{{ t('affiliate.stats.totalQuota') }}</p>
            <p class="mt-2 text-2xl font-semibold text-gray-900 ">
        ...
        <div class="card p-6">
          ...
          <p class="mt-1 text-sm text-gray-500 ">{{ t('affiliate.description') }}</p>
          ...
          <div class="mt-5 rounded-lg border border-primary-200 bg-primary-50 p-4  ">
            <p class="text-sm font-medium text-primary-800 ">{{ t('affiliate.tips.title') }}</p>
        ...
        <div class="card p-6">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-500 ">{{ t('affiliate.transfer.description') }}</p>
            </div>
            <button
        ...
        <div class="card p-6">
          ...
          <div v-if="detail.invitees.length === 0" class="mt-4 rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500  ">
