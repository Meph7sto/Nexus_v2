...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-red-100 text-red-700  '
  if (v === 'warn' || v === 'warning') return 'bg-amber-100 text-amber-700  '
  if (v === 'debug') return 'bg-slate-100 text-slate-700  '
...
<template>
  <section class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm  ">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-gray-900 ">{{ t('admin.ops.systemLogs.title') }}</h3>
        <p class="mt-1 text-xs text-gray-500 ">{{ t('admin.ops.systemLogs.description') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700  ">{{ t('admin.ops.systemLogs.queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700  ">{{ t('admin.ops.systemLogs.written') }} {{ health.written_count }}</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700  ">{{ t('admin.ops.systemLogs.dropped') }} {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700  ">{{ t('admin.ops.systemLogs.failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3  ">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-gray-700 ">{{ t('admin.ops.systemLogs.runtimeConfig') }}</div>
        ...
        <div class="md:col-span-2 xl:col-span-6">
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            ...
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
              ...
              <AdminPermissionGate resource="ops" action="execute">
        ...
        <table class="min-w-full table-fixed divide-y divide-gray-200 ">
          ...
          <tbody class="divide-y divide-gray-100 ">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-gray-700 ">{{ formatTime(row.created_at) }}</td>
              ...
              <td class="px-3 py-2 text-xs text-gray-700  whitespace-normal break-all">
