...
<template>
  <div class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-gray-900/5  ">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-gray-900 ">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-gray-500 ">{{ t('admin.ops.email.description') }}</p>
      </div>
    ...
    <div v-if="!config" class="text-sm text-gray-500 ">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-lg bg-gray-50 p-4 ">
        <h4 class="mb-2 text-sm font-semibold text-gray-900 ">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div class="text-xs text-gray-600 ">
            ...
            <span class="ml-1 font-medium text-gray-900 ">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-gray-600 ">
          ...
          <div class="text-xs text-gray-600 ">
            ...
            <span class="ml-1 font-medium text-gray-900 ">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-lg bg-gray-50 p-4 ">
        <h4 class="mb-2 text-sm font-semibold text-gray-900 ">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-gray-600 ">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-gray-900 ">
          ...
          <div class="text-xs text-gray-600 ">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-gray-900 ">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      ...
      <div class="rounded-lg bg-gray-50 p-4 ">
        <h4 class="mb-3 text-sm font-semibold text-gray-900 ">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('common.enabled') }}</div>
          ...
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <p v-if="alertRecipientError" class="mt-1 text-xs text-red-600 ">{{ alertRecipientError }}</p>
            ...
            <div class="mt-1 text-xs text-gray-500 ">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
      ...
      <div class="rounded-lg bg-gray-50 p-4 ">
        <h4 class="mb-3 text-sm font-semibold text-gray-900 ">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
          ...
          <div class="md:col-span-2">
            ...
            <p v-if="reportRecipientError" class="mt-1 text-xs text-red-600 ">{{ reportRecipientError }}</p>
            <div class="mt-2 flex flex-wrap gap-2">
              <span
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-gray-700 ">
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-gray-600 ">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-gray-500 ">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>
