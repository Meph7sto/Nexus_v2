      ...
      <OrderTable :orders="orders" :loading="ordersLoading" show-user>
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <AdminPermissionGate resource="payment_orders" action="execute">
            ...
            <AdminPermissionGate resource="payment_orders" action="execute">
              <button v-if="row.status === 'FAILED'" @click="handleRetryOrder(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50  ">
                <Icon name="refresh" size="sm" />
                {{ t('payment.admin.retry') }}
              </button>
            </AdminPermissionGate>
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700  ">{{ creditedAmountSymbol }}{{ row.refund_amount.toFixed(2) }}</span>
              <AdminPermissionGate resource="payment_orders" action="execute">
            ...
            <AdminPermissionGate resource="payment_orders" action="execute">
              <button v-if="row.status === 'REFUND_FAILED'" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50  ">
                <Icon name="refresh" size="sm" />
                {{ t('payment.admin.retryRefund') }}
              </button>
            </AdminPermissionGate>
            <AdminPermissionGate resource="payment_orders" action="execute">
              <button v-if="row.status === 'REFUND_PENDING'" :disabled="refundQueryingIds.has(row.id)" @click="handleQueryRefund(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-orange-600 hover:bg-orange-50 disabled:opacity-60  ">
                <Icon name="refresh" size="sm" :class="refundQueryingIds.has(row.id) ? 'animate-spin' : ''" />
                {{ t('payment.admin.queryRefundStatus') }}
              </button>
            </AdminPermissionGate>
            <AdminPermissionGate resource="payment_orders" action="execute">
              <button v-if="row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED'" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50  ">
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 ">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-gray-900 ">{{ creditedAmountSymbol }}{{ selectedOrder.amount.toFixed(2) }}</p></div>
          <div><p class="text-xs text-gray-500 ">{{ t('payment.orders.payAmount') }}</p><p class="text-sm font-medium text-gray-900 ">{{ paymentAmountSymbol(selectedOrder) }}{{ selectedOrder.pay_amount.toFixed(2) }}</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 ">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 ">{{ creditedAmountSymbol }}{{ selectedOrder.refund_amount.toFixed(2) }}</p></div>
          ...
          <!-- Refund request info -->
          <div v-if="selectedOrder.refund_requested_at" class="col-span-2 border-t border-gray-200 pt-3 ">
        ...
        <!-- Audit Logs -->
        <div v-if="orderAuditLogs.length > 0" class="border-t border-gray-200 pt-4 ">
          <p class="mb-2 text-xs font-medium text-gray-500 ">{{ t('payment.admin.auditLogs') }}</p>
          <div class="max-h-48 space-y-2 overflow-y-auto">
            <div v-for="log in orderAuditLogs" :key="log.id" class="rounded-lg border border-gray-100 bg-gray-50 p-2.5  ">
