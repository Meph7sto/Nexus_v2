  ...
  <AppLayout>
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-end gap-2">
        ...
        <AdminPermissionGate resource="payment_plans" action="create">
      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium text-gray-900 ">{{ planCurrencySymbol(row.currency) }}{{ (value ?? 0).toFixed(2) }}</span>
        ...
        <template #cell-for_sale="{ value, row }">
          <AdminPermissionGate resource="payment_plans" action="update">
            <button
              type="button"
              :class="[
        ...
        <template #cell-actions="{ row }">
          <div class="flex items-center gap-2">
            <AdminPermissionGate resource="payment_plans" action="update">
              <button @click="openPlanEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-blue-50 hover:text-blue-600  ">
                <Icon name="edit" size="sm" />
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
            </AdminPermissionGate>
            <AdminPermissionGate resource="payment_plans" action="delete">
    ...
    <!-- Plan Edit Dialog -->
    <AdminPermissionGate resource="payment_plans" :action="editingPlan ? 'update' : 'create'">
