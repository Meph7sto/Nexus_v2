<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-gray-200 bg-white shadow-card  "
                >
                  <div class="p-2">
                    <button
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] origin-top-right overflow-hidden rounded-lg border border-gray-200 bg-white shadow-card  "
                >
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    ...
                    <AdminPermissionGate resource="accounts" action="execute">
                    ...
                    <AdminPermissionGate resource="accounts" action="export">
                    ...
                    <AdminPermissionGate resource="error_passthrough_rules" action="view">
                    ...
                    <AdminPermissionGate resource="tls_fingerprint_profiles" action="view">
                    ...
                    <div class="grid grid-cols-1 gap-1">
                      <button
                        ...
                        class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100  "
                      >
        ...
        <div
          ...
          class="mt-2 flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800   "
        >
      ...
      <template #table>
        ...
        >
              ...
              >
                <template #trigger>
                  <a
                    ...
                    class="border-b border-dotted border-gray-300 font-medium text-gray-900  "
                  >
                    {{ value }}
                  </a>
                </template>
              </HelpTooltip>
              <span v-else class="font-medium text-gray-900 ">{{ value }}</span>
              <span
          ...
          <template #cell-schedulable="{ row }">
            <AdminPermissionGate resource="accounts" action="execute">
              <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 " :class="[row.schedulable ? 'bg-primary-500 hover:bg-primary-600' : 'bg-gray-200 hover:bg-gray-300  ']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-gray-600 ">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <AdminPermissionGate resource="accounts" action="execute">
                  <button class="text-xs px-1.5 py-0.5 rounded border border-gray-300  text-gray-600  hover:bg-gray-100 " @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
                </AdminPermissionGate>
              </div>
            </div>
          </template>
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              <div
                ...
                class="flex items-center gap-1 whitespace-nowrap text-gray-700 "
                :title="`${formatSchedulerScoreGroup(score)} / ${formatSchedulerScore(score.base_score)} / ${formatStickySchedulerScore(score)}`"
              >
                <span class="max-w-[4.75rem] truncate text-gray-500 ">{{ formatSchedulerScoreGroup(score) }}</span>
                <span class="text-gray-300 ">/</span>
                <span>{{ formatSchedulerScore(score.base_score) }}</span>
                <span class="text-gray-300 ">/</span>
                <span class="text-primary-700 ">{{ formatStickySchedulerScore(score) }}</span>
              </div>
            </div>
            <span v-else class="text-sm text-gray-400 ">-</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-gray-500 ">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <AdminPermissionGate resource="accounts" action="update">
                <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-primary-600  ">
                  <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                  <span class="text-xs">{{ t('common.edit') }}</span>
                </button>
              </AdminPermissionGate>
              <AdminPermissionGate resource="accounts" action="delete">
    ...
    <AdminPermissionGate resource="scheduled_tests" action="view">
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `nexus-account-${timestamp}.json`
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100  ;
}
