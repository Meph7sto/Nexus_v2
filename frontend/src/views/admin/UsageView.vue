<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="card">
        <div class="flex flex-wrap items-center border-b border-gray-200 px-2  sm:px-4">
          <button
        ...
        <UsageFilters v-model="filters" ref="usageFiltersRef" flat :mode="activeTab" class="border-b border-gray-100 " :start-date="startDate" :end-date="endDate" :exporting="exporting" :model-options="modelNameOptions" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
        ...
        <div v-show="activeTab === 'usage'" class="overflow-hidden rounded-b-2xl">
          <UsageTable
            ...
            :show-interaction-link="canViewUsageInteractions"
            @sort="handleSort"
...
const { can: canUsageInteractions } = useAdminPermissionGate('usage_interactions')
const canViewUsageInteractions = canUsageInteractions('view')
...
const openInteraction = (usageLogID: number) => {
  router.push({
    name: 'AdminUsageInteraction',
    params: { id: String(usageLogID) },
...
const allColumns = computed(() => [
  ...
  ...(canViewUsageInteractions.value ? [{ key: 'actions', label: t('common.actions'), sortable: false }] : [])
])
