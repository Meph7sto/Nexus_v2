  ...
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-lg border border-gray-200 bg-white py-1 shadow-card  "
                >
                  <!-- Built-in filters -->
                  <button
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'text-gray-400 ' : 'text-primary-500'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
              ...
              <button
                v-if="authStore.canAdmin('user_attributes', 'update')"
            ...
            <button
              v-if="canUpdateUsers && selectedCount > 0"
            ...
            <button v-if="canCreateUsers" @click="openCreateModal" class="btn btn-primary flex-1 md:flex-initial">
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-role="{ value }">
            <span :class="['badge', value === 'user' ? 'badge-gray' : 'badge-purple']">
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <span class="text-gray-500 ">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-card transition-opacity duration-75 group-hover/ex:opacity-100 "
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 "></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-gray-200 bg-white py-1 text-xs shadow-card  "
                >
                  <div class="border-b border-gray-100 px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-gray-400  ">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-gray-700 transition-colors hover:bg-primary-50 hover:text-primary-600   "
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-gray-400 " />
                <span class="font-medium text-gray-600 ">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-gray-400 ">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-card transition-opacity duration-75 group-hover/pub:opacity-100 ">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 "></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-gray-50 px-2 py-1 text-xs text-gray-400  "
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-gray-900 underline decoration-dashed decoration-gray-300 underline-offset-4 transition-colors hover:text-primary-600   "
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-xs text-white opacity-0 shadow-card transition-opacity duration-75 group-hover:opacity-100 ">
              ...
              <button
                v-if="canUpdateUsers && canManageUserTarget(row)"
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-emerald-600 transition-colors hover:bg-emerald-50  "
          ...
          <template #cell-balance_platform_quota="{ row }">
            <button
              v-if="canUpdateUsers && canManageUserTarget(row)"
              type="button"
              class="block text-left underline decoration-dashed decoration-gray-300 underline-offset-4 transition-colors hover:decoration-primary-400 "
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded px-1 py-0.5 transition-colors hover:bg-gray-200 "
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 min-w-[120px] rounded-lg border border-gray-200 bg-white py-1 shadow-card  "
                >
                  <button
                    v-for="metric in (['today', 'total'] as const)"
                    :key="metric"
                    type="button"
                    class="flex w-full items-center justify-between gap-3 px-3 py-1.5 text-left text-xs normal-case tracking-normal hover:bg-gray-100 "
                  ...
                  <div class="mt-1 border-t border-gray-100 px-3 py-1 text-[10px] normal-case tracking-normal text-gray-400  ">
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-900  "
                :class="{ 'bg-gray-100 text-gray-900  ': activeMenuId === row.id }"
              >
          ...
          <template #empty>
            <EmptyState
              ...
              :action-text="canCreateUsers ? t('admin.users.createUser') : undefined"
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-lg bg-white shadow-card ring-1 ring-black/5  "
        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <!-- Allowed Groups -->
              <button
                v-if="canUpdateUsers && canManageUserTarget(user)"
              ...
              <button
                v-if="canDeleteUsers && canManageUserTarget(user)"
...
const { can: canUsers } = useAdminPermissionGate('users')
const canViewUsers = canUsers('view')
const canCreateUsers = canUsers('create')
const canUpdateUsers = canUsers('update')
const canDeleteUsers = canUsers('delete')

const canManageUserTarget = (user: AdminUser): boolean =>
  user.role === 'user' || authStore.isSuperAdmin
...
const handlePlatformQuota = (user: AdminUser) => {
  if (!canUpdateUsers.value || !canManageUserTarget(user)) return
...
const handleDelete = (user: AdminUser) => {
  if (!canDeleteUsers.value || !canManageUserTarget(user)) return
  deletingUser.value = user
  showDeleteDialog.value = true
}

const confirmDelete = async () => {
  if (!deletingUser.value || !canDeleteUsers.value || !canManageUserTarget(deletingUser.value)) return
  try {
