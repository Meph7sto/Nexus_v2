        ...
        >
          ...
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              ...
              <div class="max-w-56 truncate text-sm text-gray-500 ">{{ row.out_trade_no }}</div>
            </div>
          </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-gray-100 bg-gray-50 p-4  ">
          ...
          <div class="mt-0.5 text-sm text-gray-500 ">{{ selectedOverview.username || '-' }}</div>
        </div>
    ...
    return () => h('div', { class: 'space-y-0.5' }, [
      ...
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ? 'max-w-56 truncate text-left text-sm font-medium text-primary-600 hover:text-primary-700 hover:underline  '
      ...
      h('div', { class: 'max-w-56 truncate text-sm text-gray-500 ' }, cellProps.username || '-'),
    ])
  },
})
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-gray-100 bg-white p-3  ' }, [
      h('div', { class: 'text-sm text-gray-500 ' }, statProps.label),
      h('div', {
